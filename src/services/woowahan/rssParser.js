...
class WoowahanRssParser {
  constructor() {
    ...
    this.parser = new RSSParser({
      ...
      timeout: 10000,
      headers: {
        'User-Agent': 'WoowahanTech-Bot/1.0'
      }
    });
    
    // services.json에서 feedUrl 가져오기
    const serviceConfig = getServiceByKey('woowahan');
  ...
  async parseFeed() {
    try {
      logger.info('우아한 형제들 RSS 피드 확인 시작', {
  ...
  async checkFeedHealth() {
    ...
    try {
      ...
      logger.info('우아한 형제들 피드 상태 확인 완료', result);
      return result;
  ...
  normalizeArticle(article) {
    ...
    return {
      ...processed,
      author: article.creator || article['dc:creator'] || '우아한형제들',
      category: 'Tech',
   ...
   */
  extractTags(article) {
    const tags = ['woowahan', 'backend'];
    
    // 제목에서 키워드 추출
    const title = (article.title || '').toLowerCase();
    const keywords = ['javascript', 'react', 'vue', 'node', 'backend', 'msa', 'kotlin', 'spring'];
...
module.exports = WoowahanRssParser;