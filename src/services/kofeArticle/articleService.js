const { logger, logError, logRSSActivity } = require('../../utils/logger');
const KofeRssParser = require('./rssParser');

class KofeArticleService {
  constructor() {
    this.rssParser = new KofeRssParser();
  ...
  async getNewArticles(since = null) {
    try {
      ...
      if (newArticles.length > 0) {
        logRSSActivity('Korean FE Article', `새로운 아티클 ${newArticles.length}개 발견`, {
        ...
        logRSSActivity('Korean FE Article', '새로운 아티클이 없습니다', {
          articlesCount: 0,
          trigger: 'scheduled'
        });
        return [];
      ...
      logError(error, { 
        context: 'Korean FE Article 새 아티클 조회'
      });
      return [];
    }
  }
  ...
  async getRecentArticles() {
    try {
      logger.info('Korean FE Article 체크 시작 (트리거: manual)');
      ...
      logError(error, { context: 'Korean FE Article 최근 아티클 조회' });
      return [];
    }
  }
