...
class VelogRssParser {
  constructor() {
    ...
    this.parser = new RSSParser({
      ...
      timeout: 10000,
      headers: {
        'User-Agent': 'Velog Bot 1.0'
      }
    });
    
    // services.json에서 feedUrl 가져오기
    const serviceConfig = getServiceByKey('velog');
  ...
  async parseFeed() {
    try {
      logger.info('Velog RSS 피드 확인 시작', {
  ...
  async checkFeedHealth() {
    ...
    try {
      ...
      logger.info('Velog 피드 상태 확인 완료', result);
      return result;
  ...
  normalizeArticle(article) {
    ...
    return {
      ...processed,
      author: article.creator || article['dc:creator'] || 'Velog',
      category: 'Development',
      tags: this.extractTags(article),
      source: 'Velog',
      sourceUrl: 'https://v2.velog.io/',
   ...
   */
  extractTags(article) {
    const tags = ['velog', 'development'];
    
    // 제목에서 키워드 추출
    const title = (article.title || '').toLowerCase();
    const keywords = ['javascript', 'react', 'vue', 'angular', 'typescript', 'css', 'html', 'node', 'python', 'java'];
